...
speedTest.showMarkers = function() {
  ...
  for (var i = 0; i < numMarkers; i++) {
    ...
    status.innerHTML = `<span class="label label-low">Active</span>`
    switch (speedTest.pics[i].status) {
      case 'Normal':
        status.innerHTML = `<span class="label label-green">Active</span>`
        break;
      
      case 'MediumLoad':
        status.innerHTML = `<span class="label label-yellow">Medium Load</span>`
        break;

      case 'HighLoad':
      
      case 'Full':
        status.innerHTML = `<span class="label label-red">Full</span>`
          break;
    }
    // 2 колонка - название
    title.href = '#';
