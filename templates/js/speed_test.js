...
speedTest.markerClickFunction = function (server, latlng) {
  ...
  <div style="padding-left:10px;">
      ...
      <div class="info__sector_wrap">
        ...
        <span class="label ellipsis" style="background-color:${server.color}; float: right; max-width:120px;">
        ${server.statusText}
        </span>
      </div>
      <div class="info__sector_wrap">
...
speedTest.time = function() {
  ...
  var start = new Date();
  if ($('usegmm').checked) {
    speedTest.markerClusterer = new MarkerClusterer(speedTest.map, speedTest.markers, {imagePath: '/static/images/m'});
  } else {
    for (var i = 0, marker; marker = speedTest.markers[i]; i++) {
